...
def density_speed_function(density,free_flow_speed,critical_density,mm):# fundamental diagram model (density-speed function): 
...
def vdf_calculation(internal_period_vdf_daily_link_df, vdf_index, period_index, speed_at_capacity, ultimate_capacity, critical_density, free_flow_speed, mm):    
    ...
    if DOC_RATIO_METHOD =='VBM':
        ...
        internal_period_vdf_daily_link_df['hourly_demand_over_capacity']=internal_period_vdf_daily_link_df.apply(lambda x: (ultimate_capacity+(ultimate_capacity-x.period_mean_hourly_volume_per_lane))/ultimate_capacity if x.period_mean_daily_speed<speed_at_capacity else x.period_mean_hourly_volume_per_lane/ultimate_capacity,axis=1)
        ...
        popt,pcov = curve_fit(bpr_func, x_demand_over_capacity, y_speed,p0, bounds=[lowerbound_fitting,upperbound_fitting])
        #RMSE=np.power((np.sum(np.power((bpr_func(x_demand_over_capacity, *popt)-y_speed),2))/len(x_demand_over_capacity)),0.5)
        RSE=np.sum(np.power((bpr_func(x_demand_over_capacity, *popt)-y_speed),2))/np.sum(np.power((bpr_func(x_demand_over_capacity, *popt)-y_speed.mean()),2))
        
    if DOC_RATIO_METHOD =='DBM':
        print('Density method calibration...')
        internal_period_vdf_daily_link_df['hourly_demand_over_capacity']=internal_period_vdf_daily_link_df.apply(lambda x: x.period_mean_density/critical_density,axis=1)
        X_data=[]
        Y_data=[]
        for k in range(0,len(internal_period_vdf_daily_link_df)):
    ...
    if DOC_RATIO_METHOD =='QBM':
        ...
        x_demand_over_capacity = np.array(X_data)
        y_speed = np.array(Y_data)
        popt,pcov = curve_fit(bpr_func, x_demand_over_capacity, y_speed,bounds=[lowerbound_fitting,upperbound_fitting])
    ...
    plt.plot(volume_speed_func(xvals,*popt,*popt_fundamental_diagram)/ultimate_capacity,bpr_func(xvals, *popt),c='b')
    ...
    plt.ylabel('Speed')
    plt.savefig('./3_hourly_VDF_'+DOC_RATIO_METHOD+'_'+str(vdf_index[0]+vdf_index[1]*100)+'_'+str(period_index)+'.png')    
    plt.close() 
    ...
    plt.ylabel('Speed')
    plt.savefig('./3_period_VDF_'+DOC_RATIO_METHOD+'_'+str(vdf_index[0]+vdf_index[1]*100)+'_'+str(period_index)+'.png')    
    plt.close() 
...
def vdf_calculation_daily(all_calibration_period_vdf_daily_link_results, vdf_index, speed_at_capacity, ultimate_capacity, critical_density, free_flow_speed):
    p0=np.array([free_flow_speed,0.15,4])
    ...
    popt_daily,pcov = curve_fit(bpr_func, x_demand_over_capacity, y_speed,bounds=[lowerbound_fitting,upperbound_fitting])
    #daily_RMSE=np.power((np.sum(np.power((bpr_func(x_demand_over_capacity, *popt_daily)-y_speed),2))/len(x_demand_over_capacity)),0.5)
    daily_RSE=np.sum(np.power((bpr_func(x_demand_over_capacity, *popt_daily)-y_speed),2))/np.sum(np.power((bpr_func(x_demand_over_capacity, *popt_daily)-y_speed.mean()),2))
...
def calculate_congestion_duration(speed_series,volume_per_lane_series,speed_at_capacity,ultimate_capacity):
...
if  __name__ == "__main__":
    ...
    for vdf_index,vdf_training_set in vdf_group: # vdf_index is a pair of facility type and area type e.g. vdf_index = (1,1) implies that FT=1 and AT=1 
        ...
        speed_at_capacity,ultimate_capacity,critical_density,free_flow_speed,mm=calibrate_traffic_flow_model(vdf_training_set,vdf_index) # calibrate parameters of traffic flow model
        ...
        for period_index, period_vdf_training_set in period_vdf_group: #_period_based_vdf_training_set
            ...
            internal_period_vdf_daily_link_df['period_demand_over_capacity']=internal_period_vdf_daily_link_df['period_volume'].mean()/(ultimate_capacity*period_peak_hour_factor)

            # Step 3.4 calculate alpha and beta for each period and VDF type
            calibration_period_vdf_daily_link_results = vdf_calculation(internal_period_vdf_daily_link_df, vdf_index, period_index, speed_at_capacity,
            ...
            para=[vdf_index,100*vdf_index[1]+vdf_index[0],vdf_index[0],vdf_index[1],period_index,speed_at_capacity,ultimate_capacity,critical_density,free_flow_speed,mm,period_peak_hour_factor,calibration_period_vdf_daily_link_results.alpha.mean(),calibration_period_vdf_daily_link_results.beta.mean()]
